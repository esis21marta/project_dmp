...
display_help() {
    ...
    echo
    echo "   -p, --pipeline          [ARRAY] Name of the training pipeline options [ALL | CREATE_SCAFFOLD | CUSTOMER_LOS | CUSTOMER_PROFILE | HANDSET  | HANDSET_INTERNAL | INTERNET_APP_USAGE | INTERNET_USAGE | NETWORK | PAYU_USAGE | PRODUCT | RECHARGE | REVENUE | SMS | VOICE_CALLS]"
...
then
    ...
    then
        ...
        kedro run --env=dmp/training --tag=de_primary_pipeline
        ...
        then
            ...
            kedro run --env=dmp/training --tag=prm_scaffold
        fi
        ...
        then
            ...
            kedro run --env=dmp/training --tag=prm_customer_los
        fi
        ...
        then
            ...
            kedro run --env=dmp/training --tag=prm_internet_app_usage
        fi
        ...
        then
            ...
            kedro run --env=dmp/training --tag=prm_voice_calls
        fi
    fi
    echo "###################################################################################"
...
then
    ...
    then
        ...
        kedro run --env=dmp/training --tag=de_feature_pipeline
    elif [[ (${PIPELINES[*]} == *"ALL "*) || (${PIPELINES[*]} == *" ALL"*) ]]; then
        echo "Conflicting Inputs, Pipelines to run ${PIPELINES[*]}"
        exit 1
    else
...
then
    ...
    kedro run --env=dmp/training --tag=de_master_pipeline
