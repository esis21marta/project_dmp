...
from kedro.framework.cli.utils import KedroCliError, call, forward_command, python_call
from kedro.framework.context import load_context
from kedro.utils import load_obj

from pai_clean_utils import clean_pai_path
from releases import create_release_steps, run_release_steps
from src.dmp.runner import SequentialRunnerWithRunTime, ThreadRunnerWithRunTime
from utils import GetKedroContext
from utils.search_unwanted_strings import run_search_unwanted_strings
...
DESC_ARG_HELP = """Description of the job that we are running. It will appear as
suffix in the spark application name"""
...
MAX_WORKERS_ARG_HELP = """In case of thread runner, maximum number of worker/threads/nodes
we want to execute in parallel.
"""
...
@click.option(
    "--desc", "-d", type=str, default=None, multiple=False, help=DESC_ARG_HELP
)
@click.option("--tag", "-t", type=str, multiple=True, help=TAG_ARG_HELP)
@click.option(
    "--max-workers", "-w", type=int, multiple=False, help=MAX_WORKERS_ARG_HELP
)
@click.option(
    "--load-version",
    "-lv",
...
def _run_kedro(
    ...
    if runner == "sequential" or runner == None:
        runner = SequentialRunnerWithRunTime()
    elif runner == "thread":
        runner = ThreadRunnerWithRunTime(max_workers=max_workers)
    else:
        runner = load_obj(
            runner, "kedro.runner"
        )()  # Check if max_workers need in ParallelRunner runner

    tag = _get_values_as_tuple(tag) if tag else tag
    node_names = _get_values_as_tuple(node_names) if node_names else node_names

    context = load_context(Path.cwd(), env=env, desc=desc, extra_params=params)

    GetKedroContext(context=context, env=env)
    context.run(
        tags=tag,
        runner=runner,
        node_names=node_names,
    ...
    # clean up unwanted PAI logs
    try:
        pai_run_path = context.config_loader.get("pai*", "pai*/**", "**/pai*")[
            "PAI_RUNS"
        ]
    except:
        pai_run_path = None
        print("No pai.yml is found or PAI_RUNS is not set. Skipping PAI log cleanup.")
    if pai_run_path is not None:
        clean_pai_path(pai_run_path)
...
def integration_test(params):
    log.info("=======================================================================")
    log.info("=====================  Running Integration Tests  =====================")
    log.info("=======================================================================")

    log.info("=======================================================================")
    log.info("===================  Testing Aggregation Pipeline  ====================")
    log.info("=======================================================================")
    _run_kedro(
        env="dmp_integration_test", tag=["de_aggregation_pipeline"], params=params
    )

    log.info("=======================================================================")
    log.info("=====================  Testing Remaining Pipeline  ====================")
    log.info("=======================================================================")
    _run_kedro(
        env="dmp_integration_test",
        tag=["de_primary_pipeline", "de_feature_pipeline", "de_msisdn_master_pipeline"],
        params=params,
    )

    log.info("=======================================================================")
    log.info("====================  Completed Integration Tests  ====================")
    log.info("=======================================================================")
...
def lint(files):
    """Run autoflake, isort and (on Python >=3.6) black."""
    # pylint: disable=unused-import
    if not files:
        files = ("src/tests", "src/dmp/")

    try:
        import autoflake  # fmt: off
        import isort  # fmt: off
...
def install():
    ...
    subprocess.Popen(pre_commit_install)
...
def build_docs(open_docs):
    ...
    python_call("ipykernel", ["install", "--user", "--name=dmp"])
...
def build_reqs():
    ...
    python_call("piptools", ["compile", "src/requirements.in"])
    secho(
        (
...
@cli.command("build-release-steps")
def build_release_steps():
    """Build the release steps file."""
    create_release_steps()


@cli.command("execute-release-steps")
@click.option(
    "--env",
    "-e",
    required=True,
    ...
    help="Environment on which release steps will run",
)
def execute_release_steps(env):
    """Run the release steps."""
    run_release_steps(env=env)


@cli.command("search-unwanted-strings")
@click.argument("files", type=click.Path(exists=True), nargs=-1)
def search_unwanted_strings(files):
    run_search_unwanted_strings(files)
