...
from src.dmp.pipelines.dmp._04_features.mobility.nodes_covid.fea_covid_work_cnt import (
    mobility_covid_work_cnt_features,
)
from src.tests.pysaprk_df_equality import assert_df_frame_equal


class TestFeaCovidworkCnt:
    def test_fea_covid_work_cnt(self, spark_session: SparkSession):
        ...
        actual_fea_df = mobility_covid_work_cnt_features(comm_agg_df)
        ...
        expected_fea_df = spark_session.createDataFrame(
            ...
            schema=StructType(
                [
                    ...
                    StructField("mo_id", StringType(), False),
                    StructField(
                        "fea_mobility_covid_work_cnt_mar_apr_may", IntegerType(), True
                    ),
                    StructField(
                        "fea_mobility_covid_work_cnt_apr_may_jun", IntegerType(), True
                    ),
                    StructField(
                        "fea_mobility_covid_work_cnt_may_jun_jul", IntegerType(), True
                    ),
                    StructField("fea_mobility_covid_work_cnt_mar", IntegerType(), True),
                    StructField("fea_mobility_covid_work_cnt_apr", IntegerType(), True),
                    StructField("fea_mobility_covid_work_cnt_may", IntegerType(), True),
                    StructField("fea_mobility_covid_work_cnt_jun", IntegerType(), True),
                    StructField("fea_mobility_covid_work_cnt_jul", IntegerType(), True),
                ]
            ),
        )
