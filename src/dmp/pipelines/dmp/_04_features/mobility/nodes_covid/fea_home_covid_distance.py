...
def mobility_home_covid_distance_features(
    ...
    df_home_pre_covid = (
        df_home.where(f.col("mo_id").isin("2020-01", "2020-02"))
        .distinct()
        .join(
            covid_agg_neighbor,
            (
    ...
    df_home_pre_covid_min_distance = (
        calc_distance_bw_2_lat_long(df_home_pre_covid)
    ...
    df_home_pre_covid_min_distance_latest = (
        df_home_pre_covid_min_distance.groupBy("msisdn")
        .agg(f.max("mo_id").alias("mo_id"))
        .join(df_home_pre_covid_min_distance, on=["msisdn", "mo_id"])
        .selectExpr(
            "msisdn",
    ...
    df_covid_distance = calc_distance_bw_2_lat_long(df_home_covid)
    ...
    home_covid_distance_final = (
        home_agg.selectExpr("msisdn", "mo_id")
        .distinct()
        .join(df_home_pre_covid_min_distance, on=["msisdn", "mo_id"], how="left")
        .join(df_post_covid_mar_apr_may, on=["msisdn", "mo_id"], how="left")
        .join(df_post_covid_jun_jul_aug, on=["msisdn", "mo_id"], how="left")
        .join(df_home_pre_covid_min_distance_latest, on=["msisdn"], how="left")
        .join(df_post_covid_mar_apr_may_latest, on=["msisdn"], how="left")
        .join(df_post_covid_jun_jul_aug_latest, on=["msisdn"], how="left")
        .withColumn(
            "fea_mobility_home_covid_min_distance_jan_feb",
            f.when(
                f.col("mo_id") <= f.col("mo_id_jan_feb"), f.col("min_distance_jan_feb")
            ).otherwise(f.col("min_distance_jan_feb_final")),
        )
        .withColumn(
            "fea_mobility_home_covid_avg_distance_jan_feb",
            f.when(
                f.col("mo_id") <= f.col("mo_id_jan_feb"), f.col("avg_distance_jan_feb")
            ).otherwise(f.col("avg_distance_jan_feb_final")),
        )
        .withColumn(
            "fea_mobility_home_covid_min_distance_mar_apr_may",
            f.when(
        ...
        .withColumn(
            "fea_mobility_home_covid_avg_distance_mar_apr_may",
            f.when(
                f.col("mo_id") <= f.col("mo_id_mar_apr_may"),
        ...
        .withColumn(
            "fea_mobility_home_covid_min_distance_jun_jul_aug",
            f.when(
        ...
        .withColumn(
            "fea_mobility_home_covid_avg_distance_jun_jul_aug",
            f.when(
                f.col("mo_id") <= f.col("mo_id_jun_jul_aug"),
    ...
    return home_covid_distance_final
