...
def mobility_work_covid_distance_features(
    ...
    df_work_pre_covid = (
        df_work.where(f.col("mo_id").isin("2020-01", "2020-02"))
        .distinct()
        .join(
            covid_agg_neighbor,
            (
    ...
    df_work_pre_covid_min_distance = (
        calc_distance_bw_2_lat_long(df_work_pre_covid)
    ...
    df_work_pre_covid_min_distance_latest = (
        df_work_pre_covid_min_distance.groupBy("msisdn")
        .agg(f.max("mo_id").alias("mo_id"))
        .join(df_work_pre_covid_min_distance, on=["msisdn", "mo_id"])
        .selectExpr(
            "msisdn",
    ...
    df_covid_distance = calc_distance_bw_2_lat_long(df_work_covid)
    ...
    work_covid_distance_final = (
        work_agg.selectExpr("msisdn", "mo_id")
        .distinct()
        .join(df_work_pre_covid_min_distance, on=["msisdn", "mo_id"], how="left")
        .join(df_post_covid_mar_apr_may, on=["msisdn", "mo_id"], how="left")
        .join(df_post_covid_jun_jul_aug, on=["msisdn", "mo_id"], how="left")
        .join(df_work_pre_covid_min_distance_latest, on=["msisdn"], how="left")
    ...
    return work_covid_distance_final
