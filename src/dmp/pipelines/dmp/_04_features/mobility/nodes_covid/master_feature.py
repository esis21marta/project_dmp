...
def mobility_master_features(
    ...
    df_msisdn_mo = (
        l5_staypoint_covid_monthly_stats.join(
            l5_home_covid_stay_cnt, on=["msisdn", "mo_id"], how="left"
        )
        .join(l5_covid_work_home_distance, on=["msisdn", "mo_id"], how="left")
        .join(l5_covid_work_home_not_same, on=["msisdn", "mo_id"], how="left")
        .join(l5_work_covid_stay_cnt, on=["msisdn", "mo_id"], how="left")
        .join(l5_staypoint_covid_distance, on=["msisdn", "mo_id"], how="left")
        .join(l5_work_stay_covid_distance, on=["msisdn", "mo_id"], how="left")
    )
