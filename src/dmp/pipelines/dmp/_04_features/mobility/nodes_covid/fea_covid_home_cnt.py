...
def mobility_covid_home_cnt_features(
    home_stay_point: pyspark.sql.DataFrame,
) -> pyspark.sql.DataFrame:
    df_select = home_stay_point.select(
    ...
    ).withColumn("fea_home_cnt", f.size(f.collect_set("geohash").over(w_1month)))

    df_max = (
        df_raw.groupBy("msisdn")
        .pivot("mo_id")
        .agg(f.max("fea_home_cnt").alias("max_home_cnt"))
    )
    ...
    month_max = month_raw.groupBy("msisdn").agg(
        f.max("fea_home_cnt_345").alias("fea_home_cnt_345_max"),
        f.max("fea_home_cnt_456").alias("fea_home_cnt_456_max"),
        f.max("fea_home_cnt_567").alias("fea_home_cnt_567_max"),
    )
    ...
    month_join = (
        ...
        .withColumn(
            "fea_mobility_covid_home_cnt_mar_apr_may",
            f.when(f.col("mo_id") < "2020-06", f.col("fea_home_cnt_345")).otherwise(
                f.col("fea_home_cnt_345_max")
            ),
        )
        .withColumn(
            "fea_mobility_covid_home_cnt_apr_may_jun",
            f.when(f.col("mo_id") < "2020-07", f.col("fea_home_cnt_456")).otherwise(
                f.col("fea_home_cnt_456_max")
            ),
        )
        .withColumn(
            "fea_mobility_covid_home_cnt_may_jun_jul",
            f.when(f.col("mo_id") < "2020-08", f.col("fea_home_cnt_567")).otherwise(
                f.col("fea_home_cnt_567_max")
            ),
        )
        .withColumn(
            "fea_mobility_covid_home_cnt_mar",
            f.when(f.col("mo_id") > "2020-02", f.col("2020-03")),
        )
        .withColumn(
            "fea_mobility_covid_home_cnt_apr",
            f.when(f.col("mo_id") > "2020-03", f.col("2020-04")),
        )
        .withColumn(
            "fea_mobility_covid_home_cnt_may",
            f.when(f.col("mo_id") > "2020-04", f.col("2020-05")),
        )
        .withColumn(
            "fea_mobility_covid_home_cnt_jun",
            f.when(f.col("mo_id") > "2020-05", f.col("2020-06")),
        )
        .withColumn(
            "fea_mobility_covid_home_cnt_jul",
