...
def mobility_home_poi_features(
    home_stay_agg: pyspark.sql.DataFrame, poi_agg: pyspark.sql.DataFrame,
) -> pyspark.sql.DataFrame:
    neighbor_udf = f.udf(lambda x: neighbors(x), t.ArrayType(t.StringType()))

    home_stay_point_agg = (
        home_stay_agg.where(f.col("duration_max_flag") == 1)
        .withColumn("geohash_neighbor", (neighbor_udf(home_stay_agg.geohash)))
        .withColumn("neighbor_geohash", f.explode(f.col("geohash_neighbor")))
        .drop("geohash_neighbor")
    )

    df_join = (
        home_stay_point_agg.join(
            poi_agg,
            (home_stay_point_agg.neighbor_geohash == poi_agg.poi_neighbor_geohash),
        )
        .selectExpr(
            "msisdn",
            "mo_id",
    ...
    df_cat_1_poi_features = (
        ...
        .withColumn("cat_prefix", f.lit("fea_mobility_home_cat_1").cast(t.StringType()))
        .withColumn(
            "category_renamed",
    ...
    df_cat_2_poi_features = (
        ...
        .withColumn("cat_prefix", f.lit("fea_mobility_home_cat_2").cast(t.StringType()))
        .withColumn(
            "category_renamed",
    ...
    df_final = (
        (
            home_stay_point_agg.selectExpr(
                "msisdn",
                "mo_id",
                # "geohash as home_geohash",
                # "home1_kelurahan_name",
                # "home1_neighboring_kelurahan_name",
                # "home1_kecamatan_name",
                # "home1_kabupaten_name",
