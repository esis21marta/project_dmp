...
def fea_customer_activation_outlet(
    ...
    df_fea = fea_df = (
        df_data.withColumn(
            ...
            f.last("outlet_id").over(get_rolling_window(26 * 7, oby="weekstart")),
        )
        .withColumn(
            f"fea_custprof_activation_outlet_name_06m",
            f.last("outlet_name").over(get_rolling_window(26 * 7, oby="weekstart")),
        )
        .withColumn(
            f"fea_custprof_activation_outlet_type_06m",
            f.last("outlet_type").over(get_rolling_window(26 * 7, oby="weekstart")),
        )
        .withColumn(
            f"fea_custprof_activation_latitude_06m",
            f.last("latitude").over(get_rolling_window(26 * 7, oby="weekstart")),
        )
        .withColumn(
            f"fea_custprof_activation_longitude_06m",
            f.last("longitude").over(get_rolling_window(26 * 7, oby="weekstart")),
        )
        .withColumn(
            f"fea_custprof_activation_outlet_location_type_06m",
            f.last("location_type").over(get_rolling_window(26 * 7, oby="weekstart")),
        )
        .withColumn(
            f"fea_custprof_activation_outlet_classification_06m",
            f.last("outlet_classification").over(
                get_rolling_window(26 * 7, oby="weekstart")
            ),
        )
    )
