...
def create_pipeline(**kwargs) -> Pipeline:
    """
    This function returns a pipeline which executes the necessary nodes to create the feature layer for `Handset` after the data
    ...
    Returns:
        A Pipeline object containing all the `Handset` nodes to execute the `feature` layer.
    """
    return Pipeline(
        [
            #  Note: We need to go through the base table of l2_first_2_handset_filtered, as it is missing a lot of data
            node(
                func=fea_handset,
                inputs=[
                ...
                outputs="l2_fea_handset",
                name="fea_handset",
                tags=["fea_scoring_pipeline", "scoring_pipeline"],
            ),
            node(
                func=fea_handset_imei,
                inputs=[
                    "l2_handset_time_series_filled",
                    "l1_imei_user_count",
                    "params:sla_date",
                ...
                outputs="l4_fea_handset_imei",
                name="fea_handset_imei",
            ),
        ],
        tags=["fea_handset", "handset_pipeline", "de_feature_pipeline", "de_pipeline"],
    )
