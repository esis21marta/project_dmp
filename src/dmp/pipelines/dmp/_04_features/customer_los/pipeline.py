...
from src.dmp.pipelines.dmp._04_features.customer_los.fea_customer_los import (
    fea_customer_los,
    generate_msisdn_weekstart,
)


def create_pipeline(**kwargs) -> Pipeline:
    """
    This function returns a pipeline which executes the necessary nodes to create the feature layer for `Customer LOS` after the data
    ...
    Returns:
        A Pipeline object containing all the `Customer LOS` nodes to execute the `feature` layer.
    """
    return Pipeline(
        [
            node(
                func=generate_msisdn_weekstart,
                inputs="l2_customer_los_weekly_data_final",
                outputs="l3_customer_los_msisdn_weekstart",
                name="fea_customer_los_msisdn_weekstart",
            ),
            node(
                func=fea_customer_los,
                inputs=[
                ...
                outputs="l4_fea_customer_los",
                name="fea_customer_los",
            ),
        ],
        tags=[
            "fea_customer_los",
