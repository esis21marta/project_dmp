...
from src.dmp.pipelines.dmp._01_aggregation.airtime_loan.create_loan_tracker_weekly import (
    create_loan_tracker_weekly,
)


def create_pipeline() -> Pipeline:
    """
    This function returns a pipeline which executes the necessary nodes to aggregate `airtime loan` data into a [MSISDN, Weekstart]
    ...
    Returns:
        A Pipeline object containing all the `airtime loan` nodes to execute the `aggregation` layer.
    """
    return Pipeline(
        [
            node(
                func=create_loan_tracker_weekly,
                inputs="l1_cms_data",
                outputs=None,  # "l2_airtime_loan_weekly"
                name="create_loan_tracker_weekly",
            ),
        ],
        tags=[
            "agg_airtime_loan",
