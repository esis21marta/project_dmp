...
def _weekly_aggregation(df_rech_urp: pyspark.sql.DataFrame,) -> pyspark.sql.DataFrame:
    """
    Creates Weekly Aggregation for Digital Recharge

    Args:
        df_rech_urp: Daily Recharge URP Data.

    Returns:
        Weekly aggregated digital recharge table.
    """

    df_rech_urp = df_rech_urp.withColumn(
        "event_date", f.to_date(f.col("event_date").cast(t.StringType()), "yyyy-MM-dd")
    ).withColumn("weekstart", next_week_start_day(f.col("event_date")))

    df_rech_urp_weekly = df_rech_urp.groupBy("msisdn", "weekstart").agg(
        f.sum("rech").cast(t.LongType()).alias("tot_amt_digi"),
    )

    return df_rech_urp_weekly


def create_digital_recharge_weekly(df_rech_urp: pyspark.sql.DataFrame) -> None:
    """
    Creates Weekly Aggregation for Digital Recharge
    ...
    weekly_agg_catalog = conf_catalog["l1_digi_rech_weekly"]
    ...
    while start_date < end_date:
        ...
        df_data = df_rech_urp.filter(f.col("event_date").between(sdate, edate))

        df = _weekly_aggregation(df_rech_urp=df_data).drop(f.col("weekstart"))
