...
def _weekly_aggregation(
    df_recharge_daily: pyspark.sql.DataFrame,
) -> pyspark.sql.DataFrame:
    """
    Creates Weekly Aggregation for Recharge

    Args:
        df_recharge_daily: Daily Recharge Data.

    Returns:
        Weekly aggregated recharge table.
    """
    ...
    df_recharge_weekly = df_recharge_daily.groupBy("msisdn", "weekstart").agg(
        f.sum("tot_amt").cast(t.LongType()).alias("tot_amt"),
        f.sum("tot_trx").cast(t.LongType()).alias("tot_trx"),
    )

    return df_recharge_weekly


def create_recharge_weekly(df_recharge_daily: pyspark.sql.DataFrame) -> None:
    """
    Creates Weekly Aggregation for Recharge
    ...
    weekly_agg_catalog = conf_catalog["l1_rech_weekly"]

    load_args = conf_catalog["l1_abt_rech_daily_abt_dd"]["load_args"]
    ...
    while start_date < end_date:
        ...
        df_data = df_recharge_daily.filter(
            f.col(load_args["partition_column"]).between(sdate, edate)
        )

        df = _weekly_aggregation(df_recharge_daily=df_data).drop(f.col("weekstart"))
