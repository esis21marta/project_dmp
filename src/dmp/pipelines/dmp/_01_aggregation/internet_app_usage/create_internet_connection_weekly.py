...
def _weekly_aggregation(df_bcp_usage: pyspark.sql.DataFrame) -> pyspark.sql.DataFrame:
    """
    ...
    Returns:
        Dataframe with Internet usage weekly aggregated.
    """

    df_internet_usage = (
        df_bcp_usage.withColumn(
    ...
    # Aggregating weekly
    df_internet_usage_weekly = df_internet_usage.groupBy("msisdn", "weekstart").agg(
        f.coalesce(
            f.sum(
                f.when(
        ...
        f.coalesce(
            f.sum(
                f.when(
                    (~f.col("weekend")) & (f.col("rat") == "UTRAN"),
                    f.col("volume_in").cast(t.LongType()),
                )
            ),
            f.lit(0),
        ).alias("3g_volume_in_weekday"),
        f.coalesce(
            f.sum(
                f.when(
                    f.col("weekend") & (f.col("rat") == "UTRAN"),
        ...
        f.coalesce(
            ...
            f.lit(0),
        ).alias("3g_volume_out_weekend"),
        f.coalesce(
            f.sum(
                f.when(
                    (~f.col("weekend")) & (f.col("rat") == "GERAN"),
                    f.col("volume_in").cast(t.LongType()),
                )
            ),
            f.lit(0),
        ).alias("2g_volume_in_weekday"),
        f.coalesce(
            f.sum(
                f.when(
                    f.col("weekend") & (f.col("rat") == "GERAN"),
        ...
        f.coalesce(
            ...
            f.lit(0),
        ).alias("3g_external_latency_weekday"),
        f.coalesce(
            f.count(
                f.when(
                    (~f.col("weekend")) & (f.col("rat") == "UTRAN"),
            ...
            f.lit(0),
        ).alias("3g_external_latency_count_weekday"),
        f.coalesce(
            f.sum(
                f.when(
                    f.col("weekend") & (f.col("rat") == "UTRAN"),
            ...
            f.lit(0),
        ).alias("3g_external_latency_weekend"),
        f.coalesce(
            f.count(
                f.when(
                    f.col("weekend") & (f.col("rat") == "UTRAN"),
            ...
            f.lit(0),
        ).alias("3g_external_latency_count_weekend"),
        f.coalesce(
            f.sum(
                f.when(
                    (~f.col("weekend")) & (f.col("rat") == "GERAN"),
            ...
            f.lit(0),
        ).alias("2g_external_latency_weekday"),
        f.coalesce(
            f.count(
                f.when(
                    (~f.col("weekend")) & (f.col("rat") == "GERAN"),
            ...
            f.lit(0),
        ).alias("2g_external_latency_count_weekday"),
        f.coalesce(
            f.sum(
                f.when(
                    f.col("weekend") & (f.col("rat") == "GERAN"),
            ...
            f.lit(0),
        ).alias("2g_external_latency_weekend"),
        f.coalesce(
            f.count(
                f.when(
                    f.col("weekend") & (f.col("rat") == "GERAN"),
            ...
            f.lit(0),
        ).alias("2g_external_latency_count_weekend"),
        f.coalesce(
            f.sum(
                f.when(
                    (~f.col("weekend")) & (f.col("rat") == "E-UTRAN"),
        ...
        f.coalesce(
            ...
            f.lit(0),
        ).alias("3g_internal_latency_weekday"),
        f.coalesce(
            f.count(
                f.when(
                    (~f.col("weekend")) & (f.col("rat") == "UTRAN"),
            ...
            f.lit(0),
        ).alias("3g_internal_latency_count_weekday"),
        f.coalesce(
            f.sum(
                f.when(
                    f.col("weekend") & (f.col("rat") == "UTRAN"),
            ...
            f.lit(0),
        ).alias("3g_internal_latency_weekend"),
        f.coalesce(
            f.count(
                f.when(
                    f.col("weekend") & (f.col("rat") == "UTRAN"),
            ...
            f.lit(0),
        ).alias("3g_internal_latency_count_weekend"),
        f.coalesce(
            f.sum(
                f.when(
                    (~f.col("weekend")) & (f.col("rat") == "GERAN"),
            ...
            f.lit(0),
        ).alias("2g_internal_latency_weekday"),
        f.coalesce(
            f.count(
                f.when(
                    (~f.col("weekend")) & (f.col("rat") == "GERAN"),
            ...
            f.lit(0),
        ).alias("2g_internal_latency_count_weekday"),
        f.coalesce(
            f.sum(
                f.when(
                    f.col("weekend") & (f.col("rat") == "GERAN"),
            ...
            f.lit(0),
        ).alias("2g_internal_latency_weekend"),
        f.coalesce(
            f.count(
                f.when(
                    f.col("weekend") & (f.col("rat") == "GERAN"),
            ...
            f.lit(0),
        ).alias("2g_internal_latency_count_weekend"),
        f.coalesce(
            f.sum(
                f.when(
                    (~f.col("weekend")) & (f.col("rat") == "E-UTRAN"),
        ...
        f.coalesce(
            ...
            f.lit(0),
        ).alias("2g_duration_weekend"),
    )

    return df_internet_usage_weekly


def create_internet_connection_weekly(df_bcp_usage: pyspark.sql.DataFrame) -> None:
    """
    Creates Weekly BCP Usage for the partner MSISDNs and required categories
    ...
    weekly_agg_catalog = conf_catalog["l2_internet_connection_weekly"]
    ...
    while start_date < end_date:
        ...
        df = _weekly_aggregation(df_bcp_usage=df_data).drop(f.col("weekstart"))
