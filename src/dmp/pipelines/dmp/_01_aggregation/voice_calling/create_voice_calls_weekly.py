...
def _weekly_aggregation(df_voice: pyspark.sql.DataFrame) -> pyspark.sql.DataFrame:
    """
    Aggregated voice calling data to weekly (msisdn, week)

    Args:
        df_voice: Input dataframe for voice calling

    Returns:
        df_voice_agg: Weekly aggregated voice calling dataFrame
    """
...
def create_voice_calls_weekly(df_voice: pyspark.sql.DataFrame) -> None:
    """
    Aggregated voice calling data to weekly (msisdn, week)
    ...
    weekly_agg_catalog = conf_catalog["l2_voice_calling_weekly_aggregated"]
    ...
    while start_date < end_date:
        ...
        df_data = df_voice.filter(
            f.col(load_args["partition_column"]).between(sdate, edate)
        )

        df = _weekly_aggregation(df_voice=df_data).drop(f.col("weekstart"))
