...
def _weekly_aggregation(
    df_redeemed_points: pyspark.sql.DataFrame,
) -> pyspark.sql.DataFrame:
    """
    Returns latest expiry date from a subs

    Args:
        df_redeemed_points: Daily prepaid subs base table

    Returns:
        df_redeemed_points with msisdn, weekstart and customer profile features (in select statement below)

    """

    df_redeemed_points = df_redeemed_points.withColumn(
        "event_date", f.to_date(f.col("event_date").cast(StringType()), "yyyy-MM-dd")
    ).withColumn("weekstart", next_week_start_day(f.col("event_date")))

    df_agg = df_redeemed_points.groupBy(f.col("msisdn"), f.col("weekstart")).agg(
        f.sum(f.coalesce(f.col("redeem_poin"), f.lit(0))).alias("redeem_poin"),
    )

    return df_agg


def agg_redeemed_points_to_weekly(df_redeemed_points: pyspark.sql.DataFrame,) -> None:
    """
    Fetches customer churn probability data.

    Args:
        df_redeemed_points: Customer profile dataframe (df_redeemed_points table)
    ...
    weekly_agg_catalog = conf_catalog["l2_customer_profile_points_redeemed_weekly"]

    load_args = conf_catalog["l1_dbi_redeem_poin"]["load_args"]
    ...
    while start_date < end_date:
        ...
        df_data = df_redeemed_points.filter(
            f.col(load_args["partition_column"]).between(sdate, edate)
        )

        df = _weekly_aggregation(df_redeemed_points=df_data).drop(f.col("weekstart"))
