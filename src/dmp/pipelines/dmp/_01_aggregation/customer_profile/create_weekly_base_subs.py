...
def _weekly_aggregation(df_base_subs: pyspark.sql.DataFrame,) -> pyspark.sql.DataFrame:
    """
    Returns latest expiry date from a subs

    Args:
        df_base_subs: Daily prepaid subs base table

    Returns:
        df_base_subs with msisdn, weekstart and customer profile features (in select statement below)

    """

    df_base_subs = df_base_subs.withColumn(
    ...
    df_agg = df_base_subs.groupBy(f.col("msisdn"), f.col("weekstart")).agg(
        f.max(f.col("expired_date")).alias("expected_expiry_date"),
    )

    return df_agg


def agg_customer_expiry_to_weekly(df_base_subs: pyspark.sql.DataFrame,) -> None:
    """
    Fetches customer churn probability data.

    Args:
        df_base_subs: Customer profile dataframe (df_base_subs table)
    ...
    weekly_agg_catalog = conf_catalog["l2_customer_expiry_weekly"]

    load_args = conf_catalog["l1_subs_ocs_sse"]["load_args"]
    ...
    while start_date < end_date:
        ...
        df_data = df_base_subs.filter(
            f.col(load_args["partition_column"]).between(sdate, edate)
        )

        df = _weekly_aggregation(df_base_subs=df_data).drop(f.col("weekstart"))
