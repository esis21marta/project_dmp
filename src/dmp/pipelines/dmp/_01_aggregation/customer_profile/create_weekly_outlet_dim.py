...
def _weekly_aggregation(df_outlet: pyspark.sql.DataFrame,) -> pyspark.sql.DataFrame:
    """
    Returns msisdn and the selling outlet.

    Args:
        df_outlet: Outlet Profile
    ...
    df_outlet = df_outlet.withColumn(
        "event_date", f.to_date("event_date", "yyyy-MM-dd")
    )
    df_outlet = df_outlet.withColumn(
        "weekstart", next_week_start_day(f.col("event_date"))
    )

    df_agg = df_outlet.groupBy(f.col("outlet_id"), f.col("weekstart")).agg(
        f.last(f.col("longitude")).alias("longitude"),
        f.last(f.col("lattitude")).alias("latitude"),
        f.last(f.col("namaoutlet")).alias("outlet_name"),
        f.last(f.col("kecamatan")).alias("outlet_kecamatan"),
        f.last(f.col("kabupaten")).alias("outlet_kabupaten"),
        f.last(f.col("tipe_outlet")).alias("outlet_type"),
        f.last(f.col("tipe_lokasi")).alias("location_type"),
        f.last(f.col("klasifikasi")).alias("classification"),
    )

    return df_agg


def agg_outlet_to_weekly(df_outlet: pyspark.sql.DataFrame,) -> None:
    """
    Fetches outlet profile
    ...
    weekly_agg_catalog = conf_catalog["l2_outlet_weekly"]

    load_args = conf_catalog["l1_digipos_outlet_reference_dd"]["load_args"]
    ...
    while start_date < end_date:
        ...
        df_data = df_outlet.filter(
            f.col(load_args["partition_column"]).between(sdate, edate)
        )

        log.info("start_date {}".format(start_date))
        log.info("weekstart {}".format(week_start))
        log.info("sdate {}".format(sdate))
        log.info("partition column {}".format(load_args["partition_column"]))
        df = _weekly_aggregation(df_outlet=df_data).drop(f.col("weekstart"))
