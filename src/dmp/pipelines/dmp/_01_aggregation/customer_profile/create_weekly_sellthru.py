...
def _weekly_aggregation(df_sellthru: pyspark.sql.DataFrame,) -> pyspark.sql.DataFrame:
    """
    Returns msisdn and the selling outlet.

    Args:
        df_sellthru: Transactional Table that contain msisdn and the selling outlet

    Returns:
        msisdn and selling outlet

    """

    df_sellthru = df_sellthru.withColumn(
        "weekstart", next_week_start_day(f.col("event_date"))
    )

    df_agg = df_sellthru.groupBy(f.col("msisdn"), f.col("weekstart")).agg(
        f.last(f.col("id_receiver")).alias("outlet_id"),
    )

    return df_agg


def agg_customer_profile_sellthru_to_weekly(
    df_sellthru: pyspark.sql.DataFrame,
) -> None:
    """
    Fetches msisdn and the selling outlet

    Args:
        df_sellthru: msisdn and the selling outlet
    ...
    weekly_agg_catalog = conf_catalog["l2_customer_profile_sellthru_weekly"]

    load_args = conf_catalog["l1_digipos_sp_migration"]["load_args"]
    ...
    while start_date < end_date:
        ...
        df_data = df_sellthru.filter(
            f.col(load_args["partition_column"]).between(sdate, edate)
        )

        df = _weekly_aggregation(df_sellthru=df_data).drop(f.col("weekstart"))
