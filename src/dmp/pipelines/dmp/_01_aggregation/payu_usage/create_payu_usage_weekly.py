...
def _weekly_aggregation(df: pyspark.sql.DataFrame) -> pyspark.sql.DataFrame:
    ...
    Returns:
        Weekly aggregated PAYU Usage table.
    """
    ...
    df_prep = (
        ...
        .withColumn(
            "weekend",
            f.when(f.date_format(f.col("event_date"), "u") > 5, True).otherwise(False),
        )
    )

    df_weekly_grouping = df_prep.groupBy("msisdn", "weekstart").agg(
        f.sum(f.when(f.col("weekend"), f.col("vol_data_kb"))).alias(
            "vol_data_kb_weekends"
        ),
        f.sum(f.when((~f.col("weekend")), f.col("vol_data_kb"))).alias(
            "vol_data_kb_weekdays"
        ),
        f.sum(f.col("vol_data_day_kb")).alias("vol_data_day_kb"),
        f.sum(f.col("vol_data_night_kb")).alias("vol_data_night_kb"),
        f.sum(f.col("vol_data_kb")).alias("vol_data_kb"),
        f.sum(f.when(f.col("weekend"), f.col("vol_data_day_kb"))).alias(
            "vol_data_day_kb_weekends"
        ),
        f.sum(f.when((~f.col("weekend")), f.col("vol_data_day_kb"))).alias(
            "vol_data_day_kb_weekdays"
        ),
        f.sum(f.when(f.col("weekend"), f.col("vol_data_night_kb"))).alias(
    ...
    return df_weekly_grouping


def create_payu_usage_weekly(df_payu: pyspark.sql.DataFrame) -> None:
    """
    Weekly aggregation for PayU Usage tables.
    ...
    weekly_agg_catalog = conf_catalog["l2_payu_usage_weekly_aggregation"]

    load_args = conf_catalog["l1_itdaml_dd_cdr_payu_prchse_all"]["load_args"]
    ...
    while start_date < end_date:
        ...
        df_data = df_payu.filter(
            f.col(load_args["partition_column"]).between(sdate, edate)
        )

        df = _weekly_aggregation(df=df_data).drop(f.col("weekstart"))
