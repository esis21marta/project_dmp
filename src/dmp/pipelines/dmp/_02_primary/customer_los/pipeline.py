...
def create_pipeline(**kwargs) -> Pipeline:
    """
    This function returns a pipeline which executes the necessary nodes to create the primary layer for `customer_los` after the data
    ...
    Returns:
        A Pipeline object containing all the `customer_los` nodes to execute the `primary` layer.
    """
    return Pipeline(
        [
            node(
                ...
                outputs="l2_customer_los_weekly_data_final",
                name="filter_partner_data_customer_los",
            ),
        ],
        tags=[
            "prm_customer_los",
