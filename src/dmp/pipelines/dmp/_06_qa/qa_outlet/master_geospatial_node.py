...
from utils import get_config_parameters, get_end_date
from utils.spark_data_set_helper import get_catalog_file_path


def master_geospatial_node_wrapper(project_context, geo_catalog_name):
    def master_geospatial_node(df: pyspark.sql.DataFrame) -> pyspark.sql.DataFrame:
        pipeline_mode = get_config_parameters(None, "parameters").get("pipeline")
        df = df.withColumn("weekstart", f.lit(get_end_date()))

        accuracy_completeness_metrices = get_accuracy_completeness_metrics(
            df,
            aggregate_colum="weekstart",
            non_feature_columns=["weekstart", "outlet_id"],
        ...
        file_path = get_catalog_file_path(geo_catalog_name, project_context)
        table_name = "outlet_geo"

        accuracy_completeness_metrices = add_metadata_to_output_file(
            accuracy_completeness_metrices, pipeline_mode, table_name, file_path,
        )
        outlier_metrics = add_metadata_to_output_file(
            outlier_metrics, pipeline_mode, table_name, file_path,
        )
