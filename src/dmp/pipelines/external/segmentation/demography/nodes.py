...
def clean_demography(
    demography: pyspark.sql.DataFrame, year: int, month: int
) -> pyspark.sql.DataFrame:
    """
    Selects relevant columns from the demography table and filters on the chosen year and month.
    ...
    demography = (
        demography.filter(F.col("month") == target_month)
    ...
    return demography.select(columns_of_interest)


def get_demography_cross_tabs(
    master_table: pyspark.sql.DataFrame,
) -> pyspark.sql.DataFrame:
    """
    Computes cross tabs grouping by demographics data.

    :param master_table: Table containing kr and demographics information
