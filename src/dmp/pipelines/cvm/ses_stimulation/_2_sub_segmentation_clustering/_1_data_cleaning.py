...
def select_df_to_score_or_train(
    fg_master_complete, fg_master_complete_downsampled, fg_score_or_train_tag
) -> pyspark.sql.DataFrame:
    assert fg_score_or_train_tag in ("train", "score")
    if fg_score_or_train_tag == "train":
        return fg_master_complete_downsampled
    return fg_master_complete


def to_pandas1(spark_table: pyspark.sql.DataFrame) -> pd.DataFrame:
...
def to_pandas(
    spark_table_low: pyspark.sql.DataFrame,
    spark_table_medium: pyspark.sql.DataFrame,
    spark_table_high: pyspark.sql.DataFrame,
) -> List[pd.DataFrame]:
    res_low = pd.DataFrame(spark_table_low.collect(), columns=spark_table_low.columns)
    res_medium = pd.DataFrame(
        spark_table_medium.collect(), columns=spark_table_medium.columns
    )
    res_high = pd.DataFrame(
        spark_table_high.collect(), columns=spark_table_high.columns
    )
    res = [res_low, res_medium, res_high]
...
def kmeans_clustering_fit(
    ...
    # low
    print(X_low.shape)
    if len(X_low) > max_rows:
        print("-----")
        np.random.seed(42)
        random_rows = np.random.choice(len(X_low), max_rows, False)
        X_low = X_low.loc[random_rows, :]
    print(len(X_low))
    X_low.loc[:, ses_cols_to_select_log] = np.log(
        1 + X_low.loc[:, ses_cols_to_select_log]
    )
    num_col = X_low.select_dtypes(np.number).columns.tolist()
    Xlow = X_low.drop("msisdn", axis=1)[num_col].fillna(0).as_matrix()
    km_low = km.fit(Xlow)

    # medium
    print(X_medium.shape)
    if len(X_medium) > max_rows:
        print("-----")
        np.random.seed(42)
        random_rows = np.random.choice(len(X_medium), max_rows, False)
        X_medium = X_medium.loc[random_rows, :]
    print(len(X_medium))
    X_medium.loc[:, ses_cols_to_select_log] = np.log(
        1 + X_medium.loc[:, ses_cols_to_select_log]
    )
    num_col = X_medium.select_dtypes(np.number).columns.tolist()
    Xmedium = X_medium.drop("msisdn", axis=1)[num_col].fillna(0).as_matrix()
    km_medium = km.fit(Xmedium)

    # high
    print(X_high.shape)
    if len(X_high) > max_rows:
        print("-----")
        np.random.seed(42)
        random_rows = np.random.choice(len(X_high), max_rows, False)
        X_high = X_high.loc[random_rows, :]
    print(len(X_high))
    X_high.loc[:, ses_cols_to_select_log] = np.log(
        1 + X_high.loc[:, ses_cols_to_select_log]
    )
    num_col = X_high.select_dtypes(np.number).columns.tolist()
    Xhigh = X_high.drop("msisdn", axis=1)[num_col].fillna(0).as_matrix()
    km_high = km.fit(Xhigh)

    res = [km_low, km_medium, km_high]
...
def kmeans_clustering_predict(
    ...
    num_col = X_low.select_dtypes(np.number).columns.tolist()
    Xlow = X_low.drop("msisdn", axis=1)[num_col].fillna(0).as_matrix()
    pred_clus_low = km_low.predict(Xlow)
    X_low["segment_clustering"] = pred_clus_low
    # medium
    X_medium.loc[:, ses_cols_to_select_log] = np.log(
        1 + X_medium.loc[:, ses_cols_to_select_log]
    )
    num_col = X_medium.select_dtypes(np.number).columns.tolist()
    Xmedium = X_medium.drop("msisdn", axis=1)[num_col].fillna(0).as_matrix()
    pred_clus_medium = km_medium.predict(Xmedium)
    X_medium["segment_clustering"] = pred_clus_medium
    # high
    X_high.loc[:, ses_cols_to_select_log] = np.log(
        1 + X_high.loc[:, ses_cols_to_select_log]
    )
    num_col = X_high.select_dtypes(np.number).columns.tolist()
    Xhigh = X_high.drop("msisdn", axis=1)[num_col].fillna(0).as_matrix()
    pred_clus_high = km_high.predict(Xhigh)
    X_high["segment_clustering"] = pred_clus_high

    res = [X_low, X_medium, X_high]
...
def scoring_clus(
    ...
    kmeans_low: KMeans,
    kmeans_medium: KMeans,
    kmeans_high: KMeans,
) -> List[pd.DataFrame]:

    num_col = df_low.select_dtypes(np.number).columns.tolist()
    ...
    num_col = df_low.select_dtypes(np.number).columns.tolist()
    df_low.loc[:, ses_cols_to_select_log] = np.log(
        1 + df_low.loc[:, ses_cols_to_select_log]
    )
    dflow = (
        df_low.drop("msisdn", axis=1)[num_col]
        .drop(ses_cols_to_drop, axis=1)
        .fillna(0)
        .values
    )
    cluster_pred_low = kmeans_low.predict(dflow)
    df_low["cluster"] = cluster_pred_low
    dflow = df_low[["msisdn", "cluster"]]

    df_medium = df_medium.rename(
        columns={
            "fea_int_app_usage_onlinebanking_data_vol_01m": "fea_int_app_usage_banking_data_vol_01m",
            "fea_int_app_usage_onlinebanking_data_vol_02m": "fea_int_app_usage_banking_data_vol_02m",
            "fea_int_app_usage_onlinebanking_data_vol_03m": "fea_int_app_usage_banking_data_vol_03m",
    ...
    df_medium.loc[:, ses_cols_to_select_log] = np.log(
        1 + df_medium.loc[:, ses_cols_to_select_log]
    )
    dfmedium = (
        df_medium.drop("msisdn", axis=1)[num_col]
        .drop(ses_cols_to_drop, axis=1)
        .fillna(0)
        .values
    )
    cluster_pred_medium = kmeans_medium.predict(dfmedium)
    df_medium["cluster"] = cluster_pred_medium
    dfmedium = df_medium[["msisdn", "cluster"]]
    ...
    df_high.loc[:, ses_cols_to_select_log] = np.log(
        1 + df_high.loc[:, ses_cols_to_select_log]
    )
    dfhigh = (
        df_high.drop("msisdn", axis=1)[num_col]
        .drop(ses_cols_to_drop, axis=1)
        .fillna(0)
        .values
    )
    cluster_pred_high = kmeans_high.predict(dfhigh)
    df_high["cluster"] = cluster_pred_high
    dfhigh = df_high[["msisdn", "cluster"]]

    res = [dflow, dfmedium, dfhigh]
...
def recharges_dd(
    table_a: pyspark.sql.DataFrame, table_b: pyspark.sql.DataFrame
) -> pyspark.sql.DataFrame:

    res_a = table_a.selectExpr(
        "msisdn",
        "case when voucher_type <> 1 then 'rech_package' else 'rech_balance' end rech_type",
        "trx_rech",
        "rech",
        "case when event_date between '2020-06-01' and '2020-06-30' then 1 else 0 END flag",
    )
    res_a = res_a.filter("flag=1")

    res_b = table_a.selectExpr(
        "msisdn",
        "case when rech_type like 'Pulsa%' then 'rech_package' else 'rech_balance' end rech_type",
        "trx_rech",
        "rech",
        "case when event_date between '2020-06-01' and '2020-06-39' then 1 else 0 END flag",
    )
    res_b = res_b.filter("flag=1")

    res = (
        res_a.union(res_b)
        .groupBy("msisdn", "rech_type")
        .agg(F.sum("trx_rech").alias("trx_rech"), F.sum("rech").alias("rech"))
    )
...
def summary_step1(
    ...
    table_e: pyspark.sql.DataFrame,
    table_f: pyspark.sql.DataFrame,
    table_g: pyspark.sql.DataFrame,
    cols=None,
    how="left",
) -> pyspark.sql.DataFrame:

    table_a1 = table_a.selectExpr("msisdn")
    table_b1 = table_b.selectExpr(
        "msisdn",
        "concat(lac,'-',ci) lac_ci",
        "region_lacci as regional",
        "kabupaten as city",
        "device_type",
        "status",
        "data_sdn_flag",
    ...
    table_d1 = (
        table_d.selectExpr("msisdn", "coalesce(payload_extrapolated,0) games_payload")
        .groupBy("msisdn")
        .agg(F.sum("games_payload").alias("games_payload"))
    )
    table_e1 = (
        table_e.selectExpr("msisdn", "coalesce(payload_extrapolated,0) video_payload")
        .groupBy("msisdn")
        .agg(F.sum("video_payload").alias("video_payload"))
    )
    table_e2 = (
        table_e.selectExpr(
            "msisdn",
            "coalesce(payload_extrapolated,0) youtube_payload",
            "lower(apps) apps",
        )
        .filter("apps='youtube'")
        .groupBy("msisdn")
        .agg(F.sum("youtube_payload").alias("youtube_payload"))
    )
    table_e3 = (
        table_e.selectExpr(
            "msisdn",
            "coalesce(payload_extrapolated,0) tiktok_payload",
            "lower(apps) apps",
        )
        .filter("apps='tiktok'")
        .groupBy("msisdn")
        .agg(F.sum("tiktok_payload").alias("tiktok_payload"))
    )
    table_e4 = (
        table_e.selectExpr("msisdn", "lower(apps) apps")
        .filter("apps='netflix'")
        .groupBy("msisdn")
        .agg(F.count("msisdn").alias("count_msisdn_netflix"))
    )
    table_e5 = (
        table_e.selectExpr(
            "msisdn",
            "coalesce(payload_extrapolated,0) maxstream_payload",
            "lower(apps) apps",
        )
        .filter("apps='maxstream'")
        .groupBy("msisdn")
        .agg(F.sum("maxstream_payload").alias("maxstream_payload"))
    )
    table_e6 = (
        table_e.selectExpr("msisdn", "lower(apps) apps")
        .filter("apps='viu'")
        .groupBy("msisdn")
        .agg(F.count("msisdn").alias("count_msisdn_viu"))
    )
    ...
    table_f1 = (
        table_f.selectExpr(
            ...
            "month",
            "lower(apps) apps",
            "case when msisdn like '628%' then 1 else 0 end as flag",
        )
        .filter("month = '202006' and apps = 'indihome' and flag = 1")
        .groupBy("msisdn")
        .agg(F.count("msisdn").alias("count_msisdn_indihome"))
    )
    table_f2 = (
        table_f.selectExpr(
            "msisdn",
        ...
        .filter("month = '202006' and apps = 'firstmedia' and flag = 1")
        .groupBy("msisdn")
        .agg(F.count("msisdn").alias("count_msisdn_firstmedia"))
    )
    table_g1 = table_g.selectExpr(
        "msisdn", "device_category", "dominan_recharge_channel as recharge_channel"
    )

    cols = ["msisdn"] if cols is None else cols
    table_join = (
        table_a1.join(table_b1, cols, how=how)
        .join(table_c1, cols, how=how)
        .join(table_d1, cols, how=how)
        .join(table_e1, cols, how=how)
        .join(table_e2, cols, how=how)
        .join(table_e3, cols, how=how)
        .join(table_e4, cols, how=how)
        .join(table_e5, cols, how=how)
        .join(table_e6, cols, how=how)
        .join(table_f1, cols, how=how)
        .join(table_f2, cols, how=how)
        .join(table_g1, cols, how=how)
    )
...
def summary_step2(
    ...
    res_b = table_b.selectExpr(
        "msisdn",
        "case when lower(brand) <> 'kartuhalo' and los_segment in ('02.<=6months','03.>6 months') and lower(multisim_eks) = 'y' then 1 else 0 END flag_b",
    ).filter("flag_b=1")
    res_c = table_c.selectExpr(
        "msisdn", "case when lower(multisim_int) = 'y' then 1 else 0 END flag_c"
    ).filter("flag_c=1")
    res_c = res_c.groupBy("msisdn").agg(F.count("msisdn").alias("count_msisdn_c"))

    res_d = table_d.selectExpr(
        "served_mobile_number as msisdn",
        "case when coalesce(clm_revenue,0) > 1 then 1 else 0 END flag_d",
    ).filter("flag_d=1")
    res_d = res_d.groupBy("msisdn").agg(F.count("msisdn").alias("count_msisdn_d"))

    cols = ["msisdn"] if cols is None else cols
    res_join = (
        res_a.join(res_b, cols, how=how)
        .join(res_c, cols, how=how)
        .join(res_d, cols, how=how)
    )
    res = res_join.selectExpr(
        "msisdn",
...
def summary_step3(
    ...
    res_b = (
        table_b.selectExpr("msisdn", "coalesce(payload,0) ott_payload")
        .groupBy("msisdn")
        .agg(F.sum("ott_payload").alias("ott_payload"))
    )

    res_c1 = (
        table_c.selectExpr(
            "msisdn",
            "coalesce(payload_extrapolated,0) as fb_payload",
            "case when lower(apps) in ('facebook') then 1 else 0 end as flag_fb",
        )
        .filter("flag_fb=1")
        .groupBy("msisdn")
        .agg(F.sum("fb_payload").alias("fb_payload"))
    )

    res_c2 = (
        table_c.selectExpr(
            "msisdn", "coalesce(payload_extrapolated,0) as payload_socnet_byte"
        )
        .groupBy("msisdn")
        .agg(F.sum("payload_socnet_byte").alias("payload_socnet_byte"))
    )

    res_d = (
        table_d.selectExpr("msisdn")
        .groupBy("msisdn")
        .agg(F.count("msisdn").alias("count_msisdn_bcpfintech"))
    )

    res_e = (
        table_e.selectExpr("msisdn")
        .groupBy("msisdn")
        .agg(F.count("msisdn").alias("count_msisdn_bcpbanking"))
    )

    cols = ["msisdn"] if cols is None else cols
    res_join = (
        res_a.join(res_b, cols, how=how)
        .join(res_c1, cols, how=how)
        .join(res_c2, cols, how=how)
        .join(res_d, cols, how=how)
        .join(res_e, cols, how=how)
    )
...
def summary_step4(
    ...
    res_a = table_a.selectExpr("msisdn")
    res_b1 = table_b.selectExpr(
        "msisdn", "rech as rech_balance", "trx_rech as trx_rech_balance"
    ).filter("rech_type = 'rech_balance'")
    res_b2 = table_b.selectExpr(
        "msisdn", "rech as rech_package", "trx_rech as trx_rech_package"
    ).filter("rech_type = 'rech_package'")
...
def summary_step5(
    ...
    cols = ["msisdn"] if cols is None else cols
    res = (
        table_a.join(table_b, cols, how=how)
        .join(table_c, cols, how=how)
        .join(table_d, cols, how=how)
        .join(table_e, cols, how=how)
    )
...
def summary_step6(
    ...
    table_b = (
        table_b.selectExpr(
            "lac_ci",
            "throughput_total_kbps",
            "3g_throughput_kbps",
            "total_throughput_kbps",
            "resource_block_utilizing_rate",
            "case when trx_week between '2020-21' and '2020-24' then 1 else 0 end as flag",
        )
        .filter("flag=1")
        .groupBy("lac_ci")
        .agg(
            F.avg("throughput_total_kbps").alias("throughput_total_kbps"),
            F.avg("3g_throughput_kbps").alias("3g_throughput_kbps"),
            F.avg("total_throughput_kbps").alias("total_throughput_kbps"),
            F.avg("resource_block_utilizing_rate").alias(
                "resource_block_utilizing_rate"
            ),
        )
    )
    ...
    res = res.selectExpr(
        "ses_segmentation",
        "sub_segments_named as sub_segments",
        "case\
        ...
        "los_month",
        "revenue",
        "rev_data as revenue_data_total",
        "rev_voice as revenue_voice_total",
        "rev_sms as revenue_sms_total",
        "recharge_amount",
        "recharge_transaction",
        "rech_balance as recharge_balance",
        "trx_rech_balance as recharge_transaction_balance",
        "rech_package as recharge_package",
        "trx_rech_package as recharge_transaction_package",
        "RGB as RGB_Users",
        "RGB_Data as RGB_Data_Users",
        "RGB_Voice as RGB_Voice_Users",
        "RGB_DLS as RGB_DLS_Users",
        "mytsel_user as mytsel_apps_user",
