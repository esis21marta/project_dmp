...
class ThreadRunnerWithRunTime(AbstractRunner):
    """``ThreadRunner`` is an ``AbstractRunner`` implementation. It can
    be used to run the ``Pipeline`` in parallel groups formed by toposort
    using threads.
    """
    ...
    def _log_run_time(self, node_run_time_info):
        ...
        self._logger.warn(node_run_time_info)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as pool:
            while True:
                ...
                for node in ready:
                    ...
                    future = pool.submit(run_node, node, catalog)
                    future.node_run_time_info = {
                        "node_name": node.name,
                        "node_inputs": node.inputs,
                        "node_outputs": node.outputs,
                        "node_start_time": time.time(),
                        "runner": "ThreadRunnerWithRunTime",
                    }
                    futures.add(future)
                if not futures:
                ...
                for future in done:
                    try:
                        ...
                        future.node_run_time_info["status"] = "FAILED"
                        self._log_run_time(future.node_run_time_info)
                        self._suggest_resume_scenario(pipeline, done_nodes)
                        raise
                    future.node_run_time_info["status"] = "SUCCESS"
                    self._log_run_time(future.node_run_time_info)
                    done_nodes.add(node)
